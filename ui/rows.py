...
FLOW_HOV_BG  = "#1a1730"
FLOW_SEL_BG  = "#1e1040"
FLOW_BAR     = "#a855f7"   # purple accent bar — distinct from blue app rows
FLOW_FG      = "#c084fc"
FLOW_SEL_FG  = "#e9d5ff"
FLOW_SUB_FG  = "#483d6b"
FLOW_ICON    = "⚡"        # flow identity icon
...
def draw_flow_row(parent, idx, flow: dict, is_sel: bool, on_click, on_edit):
    ...
    # ── right side first (pack right so left side doesn't crowd it) ───────
    preview = desc if desc else "  →  ".join(s.get("value", "") for s in steps[:3])
    if len(preview) > 36:
        preview = preview[:34] + "…"
    tk.Label(inner, text=preview, font=FONT_SUB, bg=bg,
             fg=sub_fg, anchor="e").pack(side="right", padx=(0, 14))

    # ── left cluster: icon | name | steps badge | alias badge ────────────
    # Use place on inner directly — no wrapper frame, so no ghost box
    ICON_X  = 10
    NAME_X  = ICON_X + 26
    BADGE_X = [NAME_X + len(name) * 7 + 8]   # mutable so we can update after
    ...
    # measure actual name pixel width after placing
    inner.update_idletasks()
    name_w = name_lbl.winfo_reqwidth()

    badge_text = f"{n_steps} step{'s' if n_steps != 1 else ''}"
    badge_lbl = tk.Label(inner, text=badge_text,
                         font=("Segoe UI", 7),
    ...
    def on_enter(e, o=outer, f=inner, i=idx):
        if i != state.selected:
            ...
            for c in f.winfo_children():
                # keep badge colours, only update plain bg labels/frames
...
def draw_flow_header(parent, on_edit_file):
    """
    Small header row above flow results:
        ⚡ Flows                          Edit flows.json
    """
    ...
    edit_lbl = tk.Label(hdr, text="Edit flows.json",
                        font=("Segoe UI", 8),
                        bg="#0e0c1a", fg="#3d2d5a",
                        cursor="hand2")
